...
const interactionPresets = {
  'none': { description: "-- Select Preset --"}, // Placeholder
  // --- NAVIGATE Presets ---
  'navigateOnClickInstant': {
    description: "Navigate on Click (Instant)",
    trigger: { type: 'ON_CLICK' },
    action: { type: 'NAVIGATE', navigation: 'INSTANT', transition: null }
  },
  'navigateOnClickDissolve': {
    description: "Navigate on Click (Dissolve)",
    trigger: { type: 'ON_CLICK' },
    action: {
      type: 'NAVIGATE',
      navigation: 'DISSOLVE',
      transition: { duration: 0.3, easing: { type: 'EASE_OUT' } },
      preserveScrollPosition: false
    }
  },
   'navigateOnClickSlideInRight': {
    description: "Navigate on Click (Slide In Right)",
    trigger: { type: 'ON_CLICK' },
    action: {
      type: 'NAVIGATE',
      navigation: 'SLIDE_IN',
      transition: { duration: 0.3, easing: { type: 'EASE_OUT' }, direction: 'LEFT' }, // Slide In Right means content moves Left
      preserveScrollPosition: false
    }
  },
  'navigateOnClickSmart': {
    description: "Navigate on Click (Smart Animate)",
    trigger: { type: 'ON_CLICK' },
    action: {
      type: 'NAVIGATE',
      navigation: 'SMART_ANIMATE',
      transition: { duration: 0.3, easing: { type: 'EASE_OUT' } },
      preserveScrollPosition: false
    }
  },
  // --- CHANGE_TO Presets ---
   'changeOnClickInstant': {
    description: "Change To on Click (Instant)",
    trigger: { type: 'ON_CLICK' },
    action: { type: 'CHANGE_TO', navigation: 'INSTANT', transition: null }
  },
  'changeOnClickDissolve': {
    description: "Change To on Click (Dissolve)",
    trigger: { type: 'ON_CLICK' },
    action: {
      type: 'CHANGE_TO',
      navigation: 'DISSOLVE',
   ...
   'changeOnClickSmart': {
    description: "Change To on Click (Smart Animate)",
    trigger: { type: 'ON_CLICK' },
    action: {
      type: 'CHANGE_TO',
      navigation: 'SMART_ANIMATE',
...
async function applyInteraction(params) {
  const selection = figma.currentPage.selection;
  if (selection.length !== 2) {
    figma.notify("Select source first, then target.", { error: true });
    return;
  }
  ...
  // --- Validate Target Node Early --- 
  if (!targetNode || !targetNode.id) {
      figma.notify("Target node is invalid or missing ID.", { error: true });
      return;
  }

  if (params.presetName && params.presetName !== 'none' && interactionPresets[params.presetName]) {
    // --- Use Preset --- 
    const preset = interactionPresets[params.presetName];
    trigger = preset.trigger;
    const presetActionDetails = preset.action; 
    console.log(`Applying preset: ${params.presetName}`);

    if (['NAVIGATE', 'CHANGE_TO'].includes(presetActionDetails.type)) {
        // Structure for NODE actions
        action = {
           type: 'NODE', 
           destinationId: targetNode.id, 
           navigation: presetActionDetails.navigation, 
           transition: presetActionDetails.transition 
        };
    } else {
        // Structure for other potential future action types
        action = Object.assign({}, presetActionDetails);
        // Add destinationId if required by other action types
        if (['OVERLAY', 'SCROLL_TO'].includes(action.type)) { 
           action.destinationId = targetNode.id;
        }
    }

  } else if (params.customSettings) {
    // --- Use Custom Settings --- 
    const settings = params.customSettings;
    console.log("Applying custom settings:", settings);
    trigger = settings.trigger; // e.g., { type: 'ON_CLICK' }
    
    const intendedActionType = settings.actionType; // 'NAVIGATE' or 'CHANGE_TO'
    const navigationType = settings.navigationType === 'NONE' ? 'INSTANT' : settings.navigationType;
    let transitionObject = null;

    // Build transition object only if navigation is NOT INSTANT
    if (navigationType !== 'INSTANT') {
        transitionObject = {
           duration: settings.duration != null ? Number(settings.duration) : 0.3, // Default duration, ensure number
           easing: { type: settings.easingType || 'EASE_OUT' } // Default easing
        };
        // Add direction only if applicable for the navigation type
        if (settings.direction && ['MOVE_IN', 'MOVE_OUT', 'SLIDE_IN', 'SLIDE_OUT', 'PUSH'].includes(navigationType)) {
             transitionObject.direction = settings.direction;
        }
    }

    if (['NAVIGATE', 'CHANGE_TO'].includes(intendedActionType)) {
        // Structure for NODE actions
        action = {
            type: 'NODE',
            destinationId: targetNode.id, 
            ...
            preserveScrollPosition: settings.preserveScroll || false 
        };
    } else {
        // Structure for other potential future action types 
        figma.notify("Custom settings for this action type are not yet supported.", { error: true });
        return; 
        // Example for a future URL action:
        // if (intendedActionType === 'URL') { 
        //    action = { type: 'URL', url: settings.urlValue };
        // } 
    }

  } else {
    figma.notify("No preset selected or invalid custom settings.", { error: true });
    return;
  }

  // --- Construct and Apply Reaction --- 
  if (!sourceNode) {
     figma.notify("Source node is invalid.", { error: true });
     return;
  }
  // --- Remove Destination ID assignment here, it's handled within NODE structure ---
  /* 
  if (['NAVIGATE', 'CHANGE_TO', 'OVERLAY', 'SCROLL_TO'].includes(action.type)) {
    if (!targetNode || !targetNode.id) {
      figma.notify("Target node is invalid or missing ID.", { error: true });
      return;
    }
  ...
  */
  ...
  const newReaction = { trigger, action };
  try {
    // Ensure sourceNode.reactions is treated as read-only if needed
    const existingReactions = sourceNode.reactions ? [...sourceNode.reactions] : [];
    existingReactions.push(newReaction);
    // Use the asynchronous method as required by Figma API
    await sourceNode.setReactionsAsync(existingReactions);
    figma.notify(`Interaction added to "${sourceNode.name}".`);
    console.log("Final Action Object:", JSON.stringify(action));
    console.log("Added reaction:", JSON.stringify(newReaction));
  } catch (error) {
    figma.notify(`Error applying interaction: ${error.message}`, { error: true });
    console.error("Error setting reactions:", error);
    console.error("Failed reaction object:", JSON.stringify(newReaction)); // Log the object that failed
  }
}
// --- End Prototyping Logic ---

// Handle UI messages
figma.ui.onmessage = async msg => {
  // --- Handle Prototype Interaction Request --- 
  if (msg.type === 'apply-prototype-interaction') {
    await applyInteraction(msg.params);
  }
  // --- Handle Update Check Request (Example) --- 
  // else if (msg.type === 'check-for-updates') {
  //   await checkForUpdates(true); // Pass flag to indicate manual check?
  // }
  // --- Existing Handlers --- 
    ...
    // Add check for Prototyping tab if needed (e.g., to send presets)
    else if (currentTab === 'prototyping-tab') {
        // Send presets to UI when tab is opened
        figma.ui.postMessage({ type: 'interaction-presets', presets: interactionPresets });
    }
  }
