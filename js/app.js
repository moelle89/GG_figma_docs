...
function scaleContainer() {
    ...
    const wrapper = container.closest(".fig-wrapper");
    if (!wrapper) return;

    const originalWidth = 900;
    const originalHeight = 444;
...
// Initialize scaling when content is loaded
...
// Add to the existing loadContent function
async function loadContent(pageName) {
    try {
        // ... existing loading code ...

        // After content is loaded and DOM is updated
        requestAnimationFrame(() => {
            // ... existing initialization code ...
            
            // Initialize scaling if we're on a page with scalable content
            if (document.getElementById("scalable-container")) {
                initializeScaling();
            }
        });
    } catch (error) {
