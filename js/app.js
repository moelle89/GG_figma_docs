...
async function loadLatestComponents() {
   const latestComponentsList = document.getElementById('latest-components-list');

   try {
      const response = await fetch('data/components.json'); // Adjust the path as necessary
      if (!response.ok) {
         throw new Error('Network response was not ok');
      }
      const components = await response.json();

      // Convert the components object to an array
      const componentsArray = Object.values(components);

      // Get the last 10 components
      const latestComponents = componentsArray.slice(-10); // Get the last 10 entries

      // Clear the list before adding new items
      latestComponentsList.innerHTML = '';

      // Display the latest components
      latestComponents.forEach(component => {
         ...
         componentItem.innerHTML = `
                <div class="component-info">
                    ...
                    <p class="component-card-description">Click to view details</p>
                </div>
            `;
         componentItem.addEventListener('click', () => {
            loadContent(component.name.toLowerCase().replace(/\s+/g, '-')); // Assuming pageName corresponds to the component name
         });
         latestComponentsList.appendChild(componentItem);
      });
   } catch (error) {
      console.error('Error loading latest components:', error);
      latestComponentsList.innerHTML = '<p class="error">Failed to load latest components.</p>';
   }
}

// Call this function when loading the latest components page
